  ...
  const renderContent = () => {
    ...
    if (view === "stats") {
      ...
      if (otherTotal > 0) {
        ...
        aggregatedDonut.push({ name: "その他", total: otherTotal, winRate: otherRate });
      }
      ...
      const myDeckData = scope === "global" && categoryMap.size > 0 ? (() => {
        ...
        if (mOtherTotal > 0) {
          const mOtherRate = (mOtherWins + mOtherLosses) > 0 ? Math.round((mOtherWins / (mOtherWins + mOtherLosses)) * 100) : null;
          kept.push({ deckName: "その他", wins: mOtherWins, losses: mOtherLosses, draws: mOtherDraws, total: mOtherTotal, winRate: mOtherRate, opponents: [] });
        }
        return kept;
      })() : stats.myDeckStats;
