...
DROP FUNCTION IF EXISTS get_global_my_deck_stats_range(date, date, text);
DROP FUNCTION IF EXISTS get_global_opponent_deck_stats_range(date, date, text);
DROP FUNCTION IF EXISTS get_global_deck_detail_stats(text, text, date, date);
DROP FUNCTION IF EXISTS get_global_opponent_deck_detail_stats(text, text, date, date);
DROP FUNCTION IF EXISTS get_deck_trend_range(date, date, text, uuid);

-- 1. get_global_my_deck_stats_range + stage filter
...
-- 2. get_global_opponent_deck_stats_range + stage filter
...
-- 3. get_global_deck_detail_stats + stage filter
...
-- 4. get_global_opponent_deck_detail_stats + stage filter
...
-- 5. get_deck_trend_range + stage filter
...
-- 6. New RPC: get_global_turn_order_stats_range (turnOrder集計をRPC化)
